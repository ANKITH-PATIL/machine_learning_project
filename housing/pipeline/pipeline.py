...
class Pipeline(Thread):
    ...
    def __init__(self,config:Configuration=Configuration())-> None:
        try:
# whenever u create an object of the pipeline class then the artifact folders of all the stages will be created 
# as we put it in inside the init function 
...
#as experiment is a class level attribute the memory allocation will be shared by all the objects
# thus it will have the same value for any object of that class
            ...
            experiment_id = str(uuid.uuid4()) # generates an unique immutable id
# we are defining a named tuple and saving it inside a csv file and consequently u can 
# read the csv file and see that u will have a running status as true and false 
...
# one pipeline will not run consequently with another pipelineas we have used the
#  thread and it can be called only once 
# though we create the object of the pipeline class but the pipeline will initialise only
#  after the experiment_run_stutus is false 